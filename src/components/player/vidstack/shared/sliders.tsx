...
export function Volume() {
  return (
    <VolumeSlider.Root
      className={`volume-slider ${styles.slider} ${styles.sliderSmall} ${styles.volumehide}`}
    >
        ...
        {(cues, forwardRef) =>
          cues.map((cue) => (
            <div
              ...
              ref={forwardRef}
            >
              <TimeSlider.Track className="ring-media-focus relative z-0 h-[5px] w-full rounded-sm bg-white/30 transition-all duration-100 group-hover/slider:h-[10px] group-data-[focus]:ring">
                <TimeSlider.TrackFill className="absolute h-full w-[var(--chapter-fill)] rounded-sm bg-white will-change-[width]" />
