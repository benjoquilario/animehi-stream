...
import { parseData } from "@/src/lib/utils/index"
...
import type { EpisodesType } from "@/src/../types/types"
...
const WatchAnime: NextPage<WatchAnimeProps> = ({
  ...
  return (
    <DefaultLayout footer={false}>
      ...
      <Section>
        <div className="grid h-full w-full grid-cols-1 gap-2 md:grid-cols-1 xl:grid-cols-5">
          <VideoPlayer
            data={animeList}
            id={animeList?.id}
          ...
          />
          <div className="col-start-1 col-end-5 row-start-2 row-end-5 h-full pb-3	md:row-start-2 md:row-end-2 xl:col-start-1 xl:col-end-2 xl:row-start-1 xl:row-end-1">
            <DetailLinks
            ...
            />
            <div className="w-full bg-background-700 p-4 text-xs text-white">
              <span>List of episode :</span>
              <DubButton dub={dub} />
            </div>
            <div className="flex h-full min-h-full flex-col overflow-auto bg-background-700 md:h-[617px] md:min-h-[617px] md:bg-[#000000eb]">
